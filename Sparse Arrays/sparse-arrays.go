...
func matchingStrings(stringsInput []string, queries []string) []int32 {
	matchs := make([]int32, len(queries))

	for i := 0; i < len(queries); i++ {
		for j := 0; j < len(stringsInput); j++ {
			if queries[i] == stringsInput[j] {
				matchs[i]++
			}
		}
	}
...
func main() {
	...
	for i := 0; i < int(stringsCount); i++ {
		...
		stringsInput = append(stringsInput, stringsItem)
	}
	...
	res := matchingStrings(stringsInput, queries)
